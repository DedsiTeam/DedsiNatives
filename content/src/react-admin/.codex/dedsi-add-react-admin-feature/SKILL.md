...
- 先读取 `content/AGENTS.md`、`content/src/react-admin/AGENTS.md`、目标目录下更近的 `AGENTS.md`、相邻业务模块和后端 API 契约。
...
7. 从 `content/src/react-admin` 运行 `bun run build`；需要时再运行 `bun run lint`。
