...
public record GetUserResponse(Guid Id, string Name, string Email, string? Phone, string? IdCardNumber,
    DateTime LastUpdatedAt, DateTime? LastLoginTime, string? LastLoginIp, DateTime? SoftDeletedAt,
    ...
    : EndpointWithoutRequest<GetUserResponse>
{
    ...
    public override void Configure()
    {
        ...
        {
            s.Summary = "获取用户详情";
            s.Description = "根据用户 ID 获取用户资料、登录信息和岗位关联。";
        });
    }
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        await Send.OkAsync(new GetUserResponse(user.Id, user.Name, user.Email, user.Phone, user.IdCardNumber,
            user.LastUpdatedAt, user.LastLoginTime, user.LastLoginIp, user.SoftDeletedAt,
            ...
            user.Positions.Select(position => new UserPositionResponse(
                position.PositionId,
                position.PositionName)).ToList()), ct);
    }
}
