...
- 先读取 `content/AGENTS.md`、Host 模块和同功能相邻 Endpoint。
...
6. 在 `HandleAsync()` 中完成编排，把业务规则留在聚合，把数据库查询留在 Query 实现。
7. 检查路由参数、筛选字段、排序、分页、空值和状态码。
8. 运行 `dotnet build`，并说明尚未执行的集成测试。

## 查询与命令

- 创建、更新、删除：注入 `I{Aggregate}Repository`，调用聚合方法后持久化。
- 详情、分页、导出：注入 `I{Aggregate}Query`，由 Infrastructure 使用 EF Core 投影 DTO 所需数据。
