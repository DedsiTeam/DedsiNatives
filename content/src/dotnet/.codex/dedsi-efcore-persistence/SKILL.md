...
- 先读取 `content/AGENTS.md`、Core 聚合和相邻 EF Core 实现。
...
- 在 Core 定义仓储和查询契约，在 Infrastructure 实现。
...
4. 实现仓储；需要复杂只读查询时实现 Query，而不是把查询写进 Endpoint。
5. 运行 `scripts/inspect-persistence.ps1` 做静态检查。
6. 从 `content/src/dotnet` 运行 `dotnet build`。
