...
- 在 `DedsiNativeInfrastructureModule.ConfigureServices` 中补充 `Configure<AbpEntityOptions>`，为每个聚合根配置 `DefaultWithDetailsFunc`。详情 Endpoint 必须通过 `Repository.GetAsync(id, true, ct)` 查询，使仓储按聚合根的默认明细配置加载完整聚合。
