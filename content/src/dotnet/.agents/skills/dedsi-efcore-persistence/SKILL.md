...
description: 按 DedsiIdentitys 项目规范实现和修改 EF Core PostgreSQL 持久化，包括实体映射、DbContext/DbSet、Dedsi 仓储、查询服务、UTC 审计字段、并发控制和数据库迁移。用于新增实体、修改字段、实现 Repository/Query 或生成检查迁移。
---
...
- 将包含 `DedsiIdentity.slnx` 的目录作为 .NET 根；先读取内容根的 `AGENTS.md`、Core 聚合和相邻 EF Core 实现。
- 把实体映射放入 `DedsiIdentity.Infrastructure/EntityFrameworkCore/Configurations/`。
- 在 `IDedsiIdentityDbContext` 与 `DedsiIdentityDbContext` 中同步维护 DbSet。
...
- 在 `DedsiIdentityInfrastructureModule.ConfigureServices` 中补充 `Configure<AbpEntityOptions>`，为每个聚合根配置 `DefaultWithDetailsFunc`。详情 Endpoint 必须通过 `Repository.GetAsync(id, true, ct)` 查询，使仓储按聚合根的默认明细配置加载完整聚合。
...
配置 `AbpEntityOptions` 时应同步核对聚合导航属性、EF Core 映射和详情响应 DTO，禁止在详情 Endpoint 中另写 `GetWithRelationsAsync` 等重复的专用加载方法。
...
6. 从 .NET 根运行 `dotnet build DedsiIdentity.slnx`。
