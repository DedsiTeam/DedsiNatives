# DedsiNative 解决方案

基于 .NET 10、ABP Framework、FastEndpoints 和 PostgreSQL 的 Clean Architecture 解决方案模板。

## 技术栈

- **后端**：.NET 10、ABP Framework、FastEndpoints、EF Core、PostgreSQL
- **服务编排**：.NET Aspire (`asipres/DedsiNative.AppHost`)
- **前端**：React 19、TypeScript、Vite、Ant Design (`react-admin/`)

## 目录结构

```text
├── src/
│   ├── DedsiNative.Core/            # 领域层：实体、领域事件、仓储接口
│   ├── DedsiNative.Infrastructure/  # 基础设施层：DbContext、EF Core 映射、仓储实现
│   └── DedsiNative.Host/            # 宿主与 API 层：FastEndpoints、ABP 模块与服务配置
├── asipres/
│   ├── DedsiNative.AppHost/         # .NET Aspire 编排项目
│   └── DedsiNative.ServiceDefaults/ # Aspire 服务默认配置（日志、指标、健康检查）
├── react-admin/                     # React 前端管理后台
├── Directory.Build.props            # 统一构建属性
└── Directory.Build.targets          # 统一构建目标
```

## 快速开始

### 1. 数据库配置与迁移

1. 检查并修改 `src/DedsiNative.Host/appsettings.json` 中的 PostgreSQL 数据库连接字符串。
2. 执行 EF Core 迁移创建数据库表：
   ```bash
   dotnet ef database update --project src/DedsiNative.Infrastructure --startup-project src/DedsiNative.Host
   ```

### 2. 启动应用

#### 方式一：使用 .NET Aspire 启动（推荐）
运行 Aspire AppHost 启动全套服务（包含后端 API 与 Aspire Dashboard）：
```bash
dotnet run --project asipres/DedsiNative.AppHost
```

#### 方式二：独立启动后端 API
```bash
dotnet run --project src/DedsiNative.Host
```

#### 方式三：启动前端项目
```bash
cd react-admin
bun install   # 或 npm install
bun dev       # 或 npm run dev
```

## 研发约束与 Agent 规范

本项目已配置通用开发规范，详见 [AGENTS.md](AGENTS.md)。
