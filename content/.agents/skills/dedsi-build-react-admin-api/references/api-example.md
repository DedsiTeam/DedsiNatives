...
export class ProductApiService {
  ...
  static getPageList(input: ProductQueryInputDto): Promise<ProductPageResultDto> {
    return request.post<ProductPageResultDto, ProductQueryInputDto>(
      '/api/product/pagedQuery',
      input,
    );
  }
  ...
  static create(input: CreateProductInputDto): Promise<string> {
    return request.post<string, CreateProductInputDto>('/api/product/create', input);
  }
  ...
  static update(id: string, input: UpdateProductInputDto): Promise<boolean> {
    return request.post<boolean, UpdateProductInputDto>(
...
必须结合当前 Axios 版本验证泛型签名。本项目响应拦截器保留 `AxiosResponse`，由 `HttpClient` 方法统一解包 `response.data`，因此 Service 直接接收 `TResponse`，不能再次访问 `.data`。
