...
- `backend`：前后端契约已确认并记录，已按变更范围加载对应 `.NET` 模块 Skill，后端实现、构建和相关测试通过。
- `frontend`：使用同一份已确认契约，已按变更范围加载对应 React 模块 Skill，类型安全的 API 接入、页面行为和前端构建通过。
...
- attempt 与结束状态
- 本轮采用的前后端契约摘要及其变更记录
- 后端和前端阶段的实际执行者；委派时记录子智能体名称，未委派时记录主代理及原因
