...
- 恢复 `in-progress` 时不增加。
- 外层执行器通过 `MaxRetries` 限制最大尝试次数。
- 达到上限的 `failed` 工作项不得继续自动处理。
...
- `backend`：已按变更范围加载并应用对应的 `.NET` 模块 Skill，领域设计已落地，后端构建和相关测试通过。
- `frontend`：已按变更范围加载并应用对应的 React 模块 Skill，类型安全的 API 接入与页面行为完成。
