...
description: 按 DedsiNative React Admin 的 Ant Design 与项目主题 Token 创建、修改和审查后台页面 UI。用于实现列表、详情、表单、弹窗、仪表盘、布局、响应式样式或纯视觉优化，并严格保持既有业务逻辑和 API 流程。
---
...
- 将包含 `package.json` 的目录作为 React 根；先读取内容根 `AGENTS.md`、`.agents/rules/react-admin.md`、`.agents/prompts/ui.md`、React 根的 `src/index.css` 和相邻页面。
...
- 优先使用 `src/index.css` 的现有 CSS 变量。间距采用 4px 基础网格并优先复用现有 `--space-*` Token；颜色、圆角和阴影的具体值均以 `src/index.css` 为准。
...
- 错误状态使用 Ant Design `message.error()` 短暂提示，不使用 `Alert` 在页面或弹窗中保留错误条幅。
...
4. 创建或修改同目录 `index.module.css`；复用变量并遵循 4px 基础网格与现有 `--space-*` Token。
